...
Defines the number of worker threads in the compression mode. In decompression mode, no matter what value is set here, the number of worker threads will always be 1.  
...
This variable enables the delivery of different to and from different threads.
...
To change the number of threads the program will use, modify the line  
`#define NUM_THREADS 8       // Define the number of threads.`  
with the number of threads the computer has. Please note that more threads will not necessarily make the program run faster.
